...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Uniforms: feed data from application to shader using uniforms
		// -------------------------------------------------------------
		float time = glfwGetTime();
		float green = sin(time) / 0.5 + 0.5;
		unsigned int uniformLocation = glGetUniformLocation(shaderProgram, "ourColor");
